...
declare
  ...
  selected_lead_source text := 'other';
  response jsonb;
  contact_id uuid;
  venue_contact jsonb;
  start_time text;
...
begin
  ...
  select contact.client_id, client.lead_source
  into selected_client_id, selected_lead_source
  from public.event_contacts as contact
  join public.clients as client on client.id = contact.client_id
  where contact.event_id = selected_event_id and contact.is_primary
  limit 1;
  ...
  set
    ...
    client_operational_notes = coalesce(nullif(response #>> '{additional,operationalNotes}', ''), client_operational_notes)
  where id = selected_event_id;

  venue_contact := response #> '{venue,assignedContact}';

  if nullif(venue_contact->>'name', '') is not null then
    insert into public.event_venue_contacts (event_id, name, role, email, phone, notes)
    values (
      selected_event_id,
      venue_contact->>'name',
  ...
  for additional_client in
    select value from jsonb_array_elements(coalesce(response #> '{client,additionalClients}', '[]'::jsonb))
  loop
    perform public.apply_questionnaire_additional_client(
      selected_event_id,
  ...
  planner_payload := response #> '{externalPlanner}';
  perform public.apply_questionnaire_planner(selected_event_id, planner_payload, 'primary_planner');

  planner_payload := response #> '{dayOfCoordinator}';
  perform public.apply_questionnaire_planner(selected_event_id, planner_payload, 'day_of_coordinator');
