-- Migration: Reset contract template bodies to the canonical versions defined in
-- other_files/contractdesign/*.html (direct_contract pages 2-3, pv_service_terms).
-- Also refreshes stored body snapshots on existing contract records.

-- 1. Update the direct_contract template body
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>{{financials.retainer_percent}}%</td>
...
<ol>
  ...
  <li>If the Client misses the deadline, the platform will automatically lock the originally estimated guest count to generate the final balance invoice.</li>
...
<ol>
  ...
  <li>Execution via the Client Portal by clicking "I Accept," executing a digital signature, or completing the checkout sequence constitutes a legally binding expression of consent per Article 89 bis of the Mexican Commercial Code.</li>
</ol>
$$
where template_key = 'direct_contract';

-- 2. Update the pv_service_terms template body
update public.contract_templates
set body = $$
...
$$
where template_key = 'pv_service_terms';

-- 3. Refresh all stored contract body snapshots from the updated template
...
where template.template_key = contract.template_key
  ...
  and contract.status in ('draft', 'sent', 'signed');
