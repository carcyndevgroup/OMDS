...
as $$
  ...
  extra_clients as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'email', client.email, 'phone', client.phone, 'role', contact.role
  ...
  prefill as (
    select jsonb_build_object(
      'client', jsonb_build_object(
        ...
        'address', trim(concat_ws(', ', nullif(primary_client.street_address, ''),
          nullif(primary_client.city, ''), nullif(primary_client.state_province, ''),
          nullif(primary_client.postal_code, ''), nullif(primary_client.country, ''))),
      ...
      'event', jsonb_build_object(
        'eventDate', event.event_date, 'eventHashtags', event.event_hashtags,
        'eventName', event.event_name, 'eventType', event.event_type,
        'guestCount', event.guest_count, 'marqueeNames', event.marquee_sign_names,
  ...
  select
    questionnaire.id, questionnaire.title, questionnaire.status,
    questionnaire.template_key,
    case
      when questionnaire.response_data <> '{}'::jsonb then questionnaire.response_data
      else prefill.data
    end,
    questionnaire.sent_at, questionnaire.submitted_at
  from portal_event as event
