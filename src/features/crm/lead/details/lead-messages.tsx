...
export function LeadMessages({ lead, t }: Props) {
  ...
  if (!threads.length && !isComposing) return <div className="space-y-3"><p className="text-sm text-zinc-400">{t("crm.lead.detail.empty.messages")}</p><button className="rounded bg-cyan-300 px-3 py-2 text-sm font-bold text-zinc-950" onClick={() => setIsComposing(true)} type="button">{t("messages.composeEmail")}</button></div>;
