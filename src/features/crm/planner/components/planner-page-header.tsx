    ...
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          {props.subtitle ? <p className="mt-2 text-sm text-zinc-400">{props.subtitle}</p> : null}
        </div>
      </div>
      {props.editHref && props.editLabel ? (
        <Link className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-cyan-300 px-4 text-sm font-bold text-zinc-950 hover:bg-cyan-200" href={props.editHref}>
