...
export function QuotePrintPage({ eventId, quoteId }: QuotePrintPageProps) {
  const { t } = useTranslation();
  const state = useEventQuotes(eventId);
  const quote = state.quotes.find((item) => item.id === quoteId);
  const version = quote?.currentVersion;

  if (state.isLoading) return <StatusMessage message={t("crm.quote.loading")} />;
  if (state.hasError || !quote || !version) {
    return <StatusMessage isError message={t("crm.quote.loadError")} />;
  }
    ...
    <main className="min-h-screen bg-zinc-100 text-zinc-950 print:bg-white">
      <div className="mx-auto flex max-w-4xl items-center justify-between gap-3 px-6 py-4 print:hidden">
        ...
        >
          ...
          {t("crm.quote.print.back")}
        </Link>
        <div className="flex items-center gap-2">
          <a
            className="inline-flex items-center gap-2 rounded-md border border-zinc-300 bg-white px-4 py-2 text-sm font-black text-zinc-800 shadow-sm transition hover:border-cyan-500 hover:text-cyan-700"
            href={`/api/crm/events/${eventId}/quotes/${quoteId}/pdf`}
          >
          ...
          >
            ...
            {t("crm.quote.print.open")}
          </button>
        </div>
      </div>
      <iframe
        className="h-[calc(100vh-5rem)] min-h-[48rem] w-full border-0"
        src={`/api/crm/events/${eventId}/quotes/${quoteId}/pdf?inline=1`}
        title={t("crm.quote.preview.documentTitle")}
      />
    </main>
  );
}
