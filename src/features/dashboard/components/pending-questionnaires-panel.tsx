...
type PanelProps = {
  hasError: boolean;
  isLoading: boolean;
  items: PendingQuestionnaireReview[];
  locale: string;
  t: ReturnType<typeof useTranslation>["t"];
};

export function PendingQuestionnairesPanel(props: PanelProps) {
  const { t } = props;
    ...
    <section className="rounded-md border border-zinc-800 bg-zinc-900 shadow-xl shadow-black/20">
      <div className="flex items-center gap-3 border-b border-zinc-800 p-5">
        ...
        <h2 className="text-xl font-bold">
          {t("dashboard.section.questionnaires")}
        </h2>
      </div>
...
function QuestionnaireContent(props: PanelProps) {
  const { hasError, isLoading, items, locale, t } = props;

  if (isLoading) {
    return <p className="p-6 text-sm text-zinc-500">{t("dashboard.loading.questionnaires")}</p>;
  }

  if (hasError) {
    return <p className="p-6 text-sm text-rose-300">{t("dashboard.error.questionnaires")}</p>;
  }

  if (!items.length) {
    return <p className="p-6 text-sm text-zinc-500">{t("dashboard.empty.questionnaires")}</p>;
  }

  return (
    <div className="divide-y divide-zinc-800">
      {items.map((item) => (
        <PendingQuestionnaireRow item={item} key={item.questionnaireId} locale={locale} t={t} />
      ))}
    </div>
  );
}

function PendingQuestionnaireRow(props: {
  item: PendingQuestionnaireReview;
  locale: string;
  t: ReturnType<typeof useTranslation>["t"];
}) {
    ...
    <article className="grid gap-4 p-5 md:grid-cols-[1fr_auto] md:items-center">
      <div className="min-w-0">
        ...
        <dl className="mt-3 grid gap-3 text-sm text-zinc-400 sm:grid-cols-2">
          <Detail label={t("dashboard.field.client")} value={item.clientName} />
          ...
          <Detail label={t("dashboard.field.submitted")} value={formatDashboardDate(item.submittedAt ?? "", locale)} />
        </dl>
      </div>
      <Link className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-cyan-300/40 px-4 text-sm font-bold text-cyan-200 transition hover:border-cyan-200" href={`/crm/clients/${item.clientId}?tab=questionnaires`}>
